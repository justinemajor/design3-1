...
float VBias = 3.3;				    // set the VBias voltage
...
float Thermistor_4th_ord_tvsv(int raw_ADC){											
    ...
    VTEMP = (VBias/ADC_BITS)*THRM_ADC;   	                                        // calculate volts per bit then multiply that times the ADV value
    THRM_TEMP = (THRM_A4*powf(VTEMP, 4)) + (THRM_A3*powf(VTEMP, 3));				// 4th order regression to get temperature
    THRM_TEMP = THRM_TEMP + (THRM_A2*powf(VTEMP, 2)) + (THRM_A1*VTEMP);
    return THRM_TEMP + THRM_A0;
}
...
int steinhart(int raw_ADC){					
	...
	t_res = t_volt/((VBias - t_volt)/RBias);		// convert the measured voltage to a resistance, the Ibias is in uA
